...
///
/// To create a guide, attach an anchor to the desired view and provide some guide configurations:
///
///		...
///
/// 	.scrollAnchor ( guides: [
///
/// 		( position: .top, axes: [ .horizontal ], alignments: [ .top ], id: "top" ),
/// 		( position: .bottom, axes: [ .horizontal ], alignments: [ .bottom ], id: "bottom" ),
/// 		( position: .center, axes: [ .vertical, .horizontal ], alignments: [ .center ], id: "center" )
///
/// 	], stackID: proxy.id )
///
@available ( iOS 16.0, * )
public struct SDSStack < Content: View > : Identifiable, View {
		...
		SDTouchInteraction { response in
			...
			GeometryReader { proxy in
				...
				self.content ( self )
					...
					.transformPreference ( SDScrollAnchors.self ) {
						
						//  Clear all associated anchors from the preference key
						
						$0 = Set ( $0.compactMap {

							//	Delete the anchor if it belongs to a this view

							if let stackID = $0.stackID, stackID == self.id { return nil } else { return $0 }
	...
	private func _scrollTo ( position: ( x: CGFloat, y: CGFloat )? = nil, translation: ( x: CGFloat, y: CGFloat )? = nil, id: ( String )? = nil, direction: UnitPoint? = nil ) -> Void {
		...
		if self.axes.contains ( .horizontal ) { withAnimation ( self.animation? ( duration ) ?? SDLibrary.Animations.expoOut ( duration: duration ) ) { self.position.x.target = mutatedPosition.x } }
		if self.axes.contains ( .vertical ) { withAnimation ( self.animation? ( duration ) ?? SDLibrary.Animations.expoOut ( duration: duration ) ) { self.position.y.target = mutatedPosition.y } }
		
	}
	
	/// Creates an ``SDSStack`` instance.
	///
	/// - Parameters:
...
private struct SDScrollAnchor: Equatable, Hashable, Identifiable {
	...
	/// Creates an ``SDScrollAnchor`` instance from an anchor source, a set of guide configurations, and a stack identifier.
	///
	/// - Parameters:
...
private struct SDScrollAnchors: PreferenceKey {
	...
	/// Creates an ``SDScrollAnchors`` key.
	///
	fileprivate init ( ) { }
...
public extension View {
	...
	///   - guides: The guide configurations for the anchor.
	///   - stackID: The identifier of the associated ``SDSStack``.
	///
	func scrollAnchor ( _ source: Anchor < CGRect > .Source = .bounds, guides configurations: [ ( position: UnitPoint, axes: [ Axis.Set ], alignments: [ UnitPoint ], id: String ) ], stackID: String? = nil ) -> some View { anchorPreference ( key: SDScrollAnchors.self, value: source ) { [ .init ( source: $0, configurations: configurations, stackID: stackID ) ] } }
